...
import { Plus, Edit, Trash2, X, Briefcase, MapPin, RefreshCw } from 'lucide-react';
...
const CareersPage = () => {
  ...
  const initialForm = {
    ...
    experience: '',
    status: 'DRAFT',
    requirements: '',     // Textarea ke liye String
    responsibilities: ''  // Textarea ke liye String
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      // String ko Array me convert karna (New Line se split karke)
      const payload = {
        ...formData,
  ...
  const openEditModal = (job) => {
    ...
    setFormData({
      ...
      status: job.status,
      // Array ko wapas String me convert karna display ke liye
          ...
          <table className="w-full text-left">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="p-5 text-xs font-semibold text-gray-500 uppercase tracking-wider">Type / Location</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {loading ? (
                <tr><td colSpan="4" className="p-10 text-center text-gray-500"><RefreshCw className="animate-spin h-6 w-6 mx-auto mb-2"/>Loading jobs...</td></tr>
              ) : jobs.length === 0 ? (
                <tr><td colSpan="4" className="p-10 text-center text-gray-500">No job openings found.</td></tr>
              ) : (
                jobs.map((job) => (
            ...
            <form onSubmit={handleSubmit} className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input type="text" className="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-blue-500 outline-none" value={formData.title} onChange={(e) => setFormData({...formData, title: e.target.value})} required />
                </div>
                ...
                <div>
                  ...
                  <input type="text" className="w-full border border-gray-300 rounded-lg p-2.5 outline-none" value={formData.location} onChange={(e) => setFormData({...formData, location: e.target.value})} required placeholder="e.g. Remote / New Delhi"/>
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Salary Range</label>
                    <input type="text" className="w-full border border-gray-300 rounded-lg p-2.5 outline-none" value={formData.salaryRange} onChange={(e) => setFormData({...formData, salaryRange: e.target.value})} placeholder="e.g. 5-8 LPA"/>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Experience</label>
                    <input type="text" className="w-full border border-gray-300 rounded-lg p-2.5 outline-none" value={formData.experience} onChange={(e) => setFormData({...formData, experience: e.target.value})} placeholder="e.g. 1-3 Years"/>
                  </div>
                </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <textarea 
                    className="w-full border border-gray-300 rounded-lg p-2.5 bg-gray-50 focus:bg-white transition-colors outline-none" 
                    rows="6" 
                    value={formData.requirements} 
                    onChange={(e) => setFormData({...formData, requirements: e.target.value})} 
                    placeholder="- Proficiency in React&#10;- Knowledge of Node.js&#10;- Good communication skills"
                  ></textarea>
                </div>
              ...
              <div className="md:col-span-2 pt-4 flex gap-3 border-t">
                ...
                <button type="submit" className="flex-1 bg-blue-600 text-white py-2 rounded-lg hover:bg-blue-700 shadow-md">
